...
if (!class_exists("DOMe")) {
    ...
    class DOMe implements Object_Array {
        ...
        // Retrieve the first element to match the tag.
        function &getElementsByTagName($tag, $levels = -1) {
            $result = array();
            if ($this->tag == $tag) {
                $result[] = $this;
            }
            ...
            // Search in order
            foreach ($this->childIndex as $key) {
                $result = array_merge($result, $this->children[$key]->getElementsByTagName($tag));
            }
        ...
        function setAttribute($attributeName, $value = "", $caseSensitive = false) {
            ...
            if (strtolower($attributeName) == "style")
                $this->buildStyles();
        ...
        function generate($tidy = true, $indent = 0, $indentStr = "  ") {
            ...
            if (!empty($this->preValue))
                $data = $indentText . $this->preValue . $tidyText;
